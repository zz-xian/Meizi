...
public class PinchImageView extends ImageView {
    ...
    //默认手势状态，值为PINCH_MODE_FREE，PINCH_MODE_SCROLL，PINCH_MODE_SCALE
    ...
    //缩放模式下图片缩放中点，该点是原图进行内层变换后的点
    private PointF mScaleCenter = new PointF();

    //缩放模式下缩放比例 = 外层缩放值（缩放后大小） / 开始缩放时两指距离
    ...
    public PinchImageView(Context context, AttributeSet attrs, int defStyle) {
        //defStyle是指它在当前Application或Activity所用Theme中的默认Style
    ...
    public Matrix getCurrentImageMatrix() {
        ...
        //a.postConcat(b)——a右乘b
        //等同setConcat(mOuterMatrix,result)
    ...
    public void setOnClickListener(OnClickListener l) {
        //默认click在任何点击情况下都会触发，so自定义
        mOnClickListener = l;
    }

    @Override
    public void setOnLongClickListener(OnLongClickListener l) {
        //默认longclick在任何长按情况下都会触发，so自定义
    ...
    //获取内部矩阵，换图之后如果图片大小不一样，会重新计算新图从而保证fit center状态
    //返回copy值
        ...
        if (getDrawable() != null) {
            ...
            if (displayWidth > 0 && displayHeight > 0) {
                ...
                if (imageWidth > 0 && imageHeight > 0) {
                    float scale;
                    //计算fit center状态（使得图片width/height能够fit控件View大小）所需的scale大小
                    if (imageWidth / imageHeight > displayWidth / displayHeight) {
                        scale = displayWidth / imageWidth;//fit width
                    } else {
                        scale = displayHeight / imageHeight;//fit height
                    }
                    //设置fit center状态的scale和位置
                    result.postScale(scale, scale, imageWidth / 2f, imageHeight / 2f);//x、y轴缩放值
                    result.postTranslate((displayWidth - imageWidth) / 2f, (displayHeight - imageHeight) / 2f);//居中
                }
            }
        }
        return result;
    ...
    //计算双击之后图片应该被缩放的比例，若值大于getMaxScale或者小于fit center尺寸，则取边界值
    ...
    //双击后放大或缩小
    //若当前缩放比例>=1，双击放大到MaxScale
    //若当前缩放比例<1，双击放大到1
    //若当前缩放比例=MaxScale，双击缩小到屏幕大小
    private void doubleTap(float x, float y) {
        ...
        //如果接下放大大于最大值或小于fit center值，则取边界
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //Action_MASK（0xff）用于处理处理多点触摸，包括ACTION_POINTER_DOWN和ACTION_POINTER_UP事件
        //&:双目运算符，将两表达式的值按二进制位展开，对应的位(bit)按值进行与运算，结果保留在该位上
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mPinchMode == PINCH_MODE_SCALE) {
                //抬起的点如果大于2，那么缩放模式还有效，但有可能初始点变了，重新测量初始点
                if (event.getPointerCount() > 2) {
                    //getAction得到的Action:低8位（即0-7位）是动作类型信息；8-15位是触控点索引信息（即表示是哪个触控点的事件）
                    //如果还没结束缩放模式，但第一个点抬起了，那么让第二个点和第三个点作为缩放控制点
                    if (event.getAction() >> 8 == 0) {//0x0001
                        saveScaleContext(event.getX(1), event.getY(1), event.getX(2), event.getY(2));
                        //如果还没结束缩放模式，但第二个点抬起了，那么让第一个点和第三个点作为缩放控制点
                    } else if (event.getAction() >> 8 == 1) {//0x0101
            ...
            //非第一个点按下，关闭滚动模式，开启缩放模式，记录缩放模式一些初始数据
    ...
    private void scaleEnd() {
        ...
        //如果缩放修正后整体导致第二层缩放小于1（就是图片比fit center状态小），重新修正缩放
    ...
    private boolean scrollBy(float xDiff, float yDiff) {
        ...
        if (bound.right - bound.left < displayWidth) {
            xDiff = 0;
            //如果图片左边在移动后超出控件左边（所谓超出，即在[0,width]区间中）
        } else if (bound.left + xDiff > 0) {
            //如果在移动前没有超出，计算应该移动的距离
            if (bound.left < 0) {
                xDiff = -bound.left;
                //否则无法移动
            } else {
            ...
            //如果图片右边在移动后超出控件右边（所谓超出，即在[0,width]区间中）
        } else if (bound.right + xDiff < displayWidth) {
            //如果在移动前没有超出，计算应该移动的距离
            if (bound.right > displayWidth) {
                xDiff = displayWidth - bound.right;//得到负值，向左移动
                //否则无法移动
            } else {
                xDiff = 0;
            }
        ...
        if (bound.bottom - bound.top < displayHeight) {
            ...
            if (bound.bottom > displayHeight) {
                yDiff = displayHeight - bound.bottom;//得到负值，向上移动
            } else {
                yDiff = 0;
            }
        }
    ...
    /**
     * 对图片进行缩放
     * @param scaleCenter 图片缩放中心，即经一层变换后的左边
     * @param scaleBase   缩放比例
     * @param distance    新缩放点距离
    ...
    private void scale(PointF scaleCenter, float scaleBase, float distance, PointF lineCenter) {
        ...
        //按照图片缩放中心缩放，并让缩放中心在缩放点中点上
        matrix.postScale(scale, scale, scaleCenter.x, scaleCenter.y);
        matrix.postTranslate(lineCenter.x - scaleCenter.x, lineCenter.y - scaleCenter.y);
        //应用变换
        mOuterMatrix = matrix;
        onOuterMatrixChanged();
        //重绘（刷新View，把旧View从主UI线程队列中pop掉）
        invalidate();
    }
    ...
    private class FlingAnimator extends ValueAnimator implements ValueAnimator.AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            mVector[0] *= FLING_DAMPING_FACTOR;//滑动抑制参数
    ...
    private static class MathUtils {
        ...
        public static float[] getMatrixScale(Matrix matrix) {
            if (matrix != null) {
                ...
                matrix.getValues(value);//获得3*3矩阵
        ...
        public static float[] inverseMatrixPoint(float[] point, Matrix matrix) {
            if (point != null && matrix != null) {
                ...
                //得到当前坐标对应matrix操作之前的坐标：先将当前矩阵进行逆矩阵再进行mapPoints()
                //invert(Matrix inverse)——得到当前矩阵的逆矩阵，存入inverse
                //mapPoints(float[] dst, float[] src)——用当前矩阵改变src的值，存入dst
